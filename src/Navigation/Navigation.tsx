...
export default function Navigation() {
  ...
  const handleChange = useCallback((event: ChangeEvent<{}>, id: number) => {
    if (id === 0) deleteGraph()

    setOpenTabId(id)
  }, [])
  
  const addNewGraph = useCallback((graphAdjacencyMatrix: number[][]) => {
    ...
    <>
      <Paper color='default' classes={{ root: styles.root }} square>
        <Tabs value={openTabId} onChange={handleChange} centered indicatorColor='primary'>
          ...
          />
          <Tab label='Prezentacja grafu' classes={{ root: styles.tab }} disabled={!isGraphLoaded} />
        </Tabs>
      </Paper>
